...
if [ "${policy_library_sync_gcs_enabled}" == "true" ]; then
...
elif [ "${policy_library_sync_enabled}" == "true" ]; then
  echo "Forseti Startup - Using git-sync to sync Policy Library from ."
  ...
  sleep 10
elif [ -n "${policy_library_bundle}" ]; then
  ...
  # TODO: Uncomment when kpt setters are published
